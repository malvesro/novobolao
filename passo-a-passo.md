...
7. **[Em Progresso] Auditoria de Acessibilidade:** Inventário inicial realizado em 20/02/2026 com foco em WCAG 2.1 AA. Correções prioritárias aplicadas em 20/02/2026: inclusão de `alt` descritivo nas bandeiras, IDs únicos nos filtros de jogos, diálogos de palpites com `role="dialog"`/gerenciamento de foco, mensagens com `aria-live` e landmarks (`skip link`, `role="main"`/`contentinfo`). Referências logs: `.ia/logs/session-20260220-auditoria-acessibilidade.md`, `.ia/logs/session-20260220-acessibilidade-correcoes.md`. Próximas etapas: rodar auditoria automatizada (axe/Lighthouse) e ajustar contrastes/feedbacks restantes para atingir score > 90.
