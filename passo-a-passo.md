...
2.  **[Em Progresso] Inventário e Análise de Scripts:** Mapear todos os arquivos JavaScript, identificar dependências e decidir manter/refatorar/remover cada um. Resultado alimentará as tarefas 3 e 4.
    ...
    *   **Próximas subtarefas:**
        ...
        6. **[Em Progresso]** Propor adoção de bundler (Vite/ESBuild) para modularizar scripts, permitir CSP rígida e preparar minificação/versões com hash. Plano criado em 20/02/2026 (`.ia/planos/plano-bundler-frontend.md`) com log de sessão `.ia/logs/session-20260220-plano-bundler-frontend.md`. Estrutura inicial (`package.json`, `vite.config.js`, `src/frontend/`) e bundle fallback documentados em `.ia/logs/session-20260220-bundler-setup-parcial.md`. **Atualização 20/02/2026:** `package.json` atualizado com `engines`/`packageManager` e `pom.xml` configurado com `frontend-maven-plugin` (executado em `generate-resources`, `frontend.skip=true` por padrão). Execução `npm install --progress=false --verbose` falhou por `EAI_AGAIN` (falta de resolução DNS para `registry.npmjs.org`); manter fallback manual até liberar rede. `vite.config.js` passou a gerar `manifest.json` para suportar bundles versionados assim que o build for possível, e `webapp/template/cabecalho.jspf` agora carrega dinamicamente o bundle via manifest com fallback seguro. Referências log: `.ia/logs/session-20260220-bundler-pipeline-config.md`, `.ia/logs/session-20260220-bundler-manifest-config.md`, `.ia/logs/session-20260220-bundler-manifest-loader.md`.
